...
const Month = React.memo(({ monthIndex, journalDates, currentYear, months, daysOfWeek }) => {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    days.push(
      ...
      >
        <div className={`w-6 h-6 flex items-center justify-center ${isToday ? 'bg-blue-200 rounded-full' : ''}`}>
          <span className={`${isToday ? 'text-blue-600 font-semibold' : 'text-gray-700'}`}>{day}</span>
        </div>
  ...
  return (
    <div className="flex flex-col h-full">
      <h3 className="text-base font-semibold text-neutral-800 text-center mb-2">{months[monthIndex]}</h3>
    ...
    <div className="min-h-screen bg-white p-4">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-center gap-4 mb-6">
          <button onClick={() => setCurrentYear(currentYear - 1)} className="p-2 rounded transition-colors duration-200 hover:bg-neutral-200">
            <ChevronLeft size={20} className="text-gray-700" />
          </button>
          <h1 className="text-3xl font-bold min-w-[140px] text-center">{currentYear}</h1>
          <button onClick={() => setCurrentYear(currentYear + 1)} className="p-2 rounded transition-colors duration-200 hover:bg-neutral-200">
